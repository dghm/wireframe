...
const main = async () => {
  console.log('🚢 開始編譯 TailorMed Wireframe...');
    ...
    const compilePugRecursive = (dir, relativePath = '') => {
      ...
      entries.forEach((entry) => {
        ...
        if (entry.isDirectory()) {
          ...
          // 確定輸出目錄
          let outputDir = PROJECT_DIST;
          if (relativePath) {
            outputDir = path.join(PROJECT_DIST, relativePath);
            ensureDir(outputDir);
          }
          ...
          const displayPath = relativePath 
            ? `dist/Projects/TailorMed/${relativePath}/${outputName}`
            : `dist/Projects/TailorMed/${outputName}`;
  ...
  try {
    ...
    if (stylusFiles.length === 0) {
      console.warn('⚠️ TailorMed: 找不到 Stylus 檔案');
    }
    ...
    for (const file of stylusFiles) {
      ...
      console.log(`  ✅ 生成 dist/Projects/TailorMed/css/${outputName}`);
    }
  } catch (error) {
  ...
  console.log('\n🎉 TailorMed Wireframe 編譯完成！');
  console.log('📁 預覽路徑：dist/Projects/TailorMed/index.html');
};
