...
try {
  ...
  pugFiles.forEach((file) => {
    ...
    console.log(
      `  ✅ 已生成 dist/Projects/TailorMed/shipmentTracking/${outputName}`
    );
  });
} catch (error) {
  ...
  stylusFiles.forEach((file) => {
    ...
    stylus(stylusCode)
      ...
      .render((err, css) => {
        ...
        console.log(
          `  ✅ 已生成 dist/Projects/TailorMed/shipmentTracking/css/${outputName}`
        );
      });
  });
} catch (error) {
...
console.log(
  '🎉 編譯完成！可以在 dist/Projects/TailorMed/shipmentTracking/index.html 預覽貨件追蹤 MVP'
);
